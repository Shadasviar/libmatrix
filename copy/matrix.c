...
void walk_on_matrix(MODYFIED matrix*, action, double param);
...
void walk_on_two_matrixes(IN matrix*, OUT matrix*, action_with_two_matrix);
...
void triangle_form_of_augmented_matrix(IN matrix*, OUT matrix*, OUT matrix*);
...
void show_matrix(IN matrix *in_matrix){
  walk_on_matrix(in_matrix, show, UNUSED_PARAM);
}


void init_matrix(OUT matrix *out_matrix){
  walk_on_matrix(out_matrix, init, UNUSED_PARAM);
}


void init_matrix_by_random(OUT matrix *out_matrix){ 
  walk_on_matrix(out_matrix, init_by_random, UNUSED_PARAM);
}


void init_matrix_as_unit(OUT matrix *out_matrix){
  walk_on_matrix(out_matrix, init_as_unit, UNUSED_PARAM);
}


void transponent(IN matrix *in_matrix, OUT matrix *out_matrix){
  matrix result = make_matrix(in_matrix->n_columns, in_matrix->n_rows);
  walk_on_two_matrixes(in_matrix, &result, transpon);
  copy_matrix(&result, out_matrix);
  delete_matrix(&result);
}


void multiplex_matrixes(IN matrix *in_matrix_1, IN matrix *in_matrix_2, OUT matrix *out_matrix){
  if(in_matrix_2->n_rows == in_matrix_1->n_columns){
    ...
    puts("multiplex_matrixes: ERROR: THEESE MATRIXES CANNOT BE MULTIPLEXED");
    exit(1);
  }
}
...
void copy_matrix(IN matrix *in_matrix, OUT matrix *out_matrix){
     ...
     && matrix_exists(in_matrix))
  {
    ...
    *out_matrix = make_matrix(in_matrix->n_rows, in_matrix->n_columns);
    walk_on_two_matrixes(in_matrix, out_matrix, copy_element);
  }
  else ;
}


void delete_matrix(matrix *in_matrix){
  for(int i = 0; i < in_matrix->n_rows; i++){
    if(in_matrix->array){
...
void rows_swap(const int i_row_1, const int i_row_2, MODYFIED matrix *in_matrix){
  if(indexes_are_right(i_row_1, i_row_2, in_matrix->n_rows)){
    ...
    puts("rows_swap: "OUT_OF_RANGE_MATRIX_ERR);
  }
}


void columns_swap(const int i_col_1, const int i_col_2, MODYFIED matrix *in_matrix){
  transponent(in_matrix, in_matrix);
  rows_swap(i_col_1, i_col_2, in_matrix);
  transponent(in_matrix, in_matrix);
}


void triangle_form(IN matrix *in_matrix, OUT matrix *out_matrix){
  triangle_form_of_augmented_matrix(in_matrix, out_matrix, NULL);
}


void row_mult_on_const(const double factor, const int i_row, MODYFIED matrix *out_matrix){
  if(indexes_are_right(i_row, UNUSED_PARAM, out_matrix->n_rows) && matrix_exists(out_matrix)){
    ...
    walk_on_matrix(&tmp, mult, factor);
  }
  else{
    puts("row_mult_on_const: "OUT_OF_RANGE_MATRIX_ERR);
  }
}


void column_mult_on_const(const double factor, const int i_column, MODYFIED matrix *out_matrix){
  transponent(out_matrix, out_matrix);
  row_mult_on_const(factor, i_column, out_matrix);
  transponent(out_matrix, out_matrix);
}
...

{
  if(indexes_are_right(i_subtracted_row, i_subtracting_row, out_matrix->n_rows) && matrix_exists(out_matrix)){
    ...
    puts("rows_sub: "OUT_OF_RANGE_MATRIX_ERR);
  }
}
...
{
  transponent(out_matrix, out_matrix);
  rows_sub(factor, i_subtracted_column, i_subtracting_column, out_matrix);
  transponent(out_matrix, out_matrix);
}


void copy_row_to_other_matrix(
  const int i_source,
  ...
  OUT matrix *out_matrix )
{
    ...
    && in_matrix->n_columns == out_matrix->n_columns)
  {
    ...
    puts("copy_row_to_other_matrix: "GENERAL_ERR);
  }
}


void copy_column_to_other_matrix(
  const int i_source,
  const int i_reciever,
  IN matrix *in_matrix,
  OUT matrix *out_matrix )
{
  transponent(in_matrix, in_matrix);
  transponent(out_matrix, out_matrix);
  copy_row_to_other_matrix(i_source, i_reciever, in_matrix, out_matrix);
  transponent(in_matrix, in_matrix);
  transponent(out_matrix, out_matrix);
}


void inverse_matrix(IN matrix *in_matrix, OUT matrix *out_matrix){
  if(matrix_exists(in_matrix)
...
void triangle_form_of_augmented_matrix(IN matrix *in_matrix, OUT matrix *out_matrix, OUT matrix *half_an_invert_matrix){
...
void walk_on_matrix(MODYFIED matrix *in_matrix, action foo,const double param){
  if(matrix_exists(in_matrix)){
    for(int i = 0; i < in_matrix->n_rows; i++){
    ...
    puts(UNEXISTING_MATRIX_ERR);
  }
}


void walk_on_two_matrixes(IN matrix *in_matrix, OUT matrix *out_matrix, action_with_two_matrix foo){
